...
testRAs,testDecs = ut.nsideToDeclRa(16)
...
class TestIonoMap(unittest.TestCase):
    ...
    def test_radec_single(self):
        self.rm_map.get_radec_RM([testRA],[testDec])
        assert(self.rm_map.RMs.shape == (1,24))
        assert(self.rm_map.dRMs.shape == (1,24))
        #Plot the comparison plot for Sotomayor Beltran et al. 2013 Fig.4b        
        plt.errorbar(range(24), self.rm_map.RMs[0,:], yerr=self.rm_map.dRMs[0,:], fmt='ro', ecolor='r')
